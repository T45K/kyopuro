package AtCoder.other.typecal90.bh060;
...
public class Main {
  public static void main(final String[] args) {
    ...
    final int[] natural = count(list);
    Collections.reverse(list);
    final int[] reverse = count(list);
    final int answer = IntStream.range(0, n)
        .map(i -> natural[i] + reverse[n - i - 1] - 1)
        .max()
        .orElseThrow();
    System.out.println(answer);
  }

  private static int[] count(final List<Integer> list) {
    final SegmentTree<Integer> tree = new SegmentTree<>(list.size() + 1, 0, Math::max);
    return list.stream()
        .map(value -> new Pair(value, tree.query(0, value) + 1))
        .peek(pair -> tree.update(pair.value, pair.max))
        .mapToInt(pair -> pair.max)
        .toArray();
  }
  ...
  private static class Pair {
    ...
    Pair(final int value, final int max) {
      this.value = value;
      this.max = max;
    }
  }
