...
public class Main {
    ...
    public static void main(final String[] args) {
        ...
        final CombinationCalculator calculator = new CombinationCalculator(n + m + 1, MOD);
        long all = calculator.calc(n + m, n);
        for (int i = 0; i <= m && n > i + k; i++) {
            all -= calculator.calc(n + i - 1, i);
            all += MOD;
            all %= MOD;
        }
